...
from lexnetic_school.schemas import\
	...
	SchoolOut, SchoolIn, SchoolPatch,\
	ClassOut, ClassIn, ClassPatch,\
	TeacherOut, TeacherIn, TeacherPut, TeacherPatch,\
	StudentOut, StudentIn, StudentPut, StudentPatch,\
	MemberOut, MemberIn, MemberPatch,\
	PersonalInfoOut, PersonalInfoIn, PersonalInfoPatch
...
HEADMASTER_IN_1_SCH_1 = HeadMasterIn(
			school_id="1",
			username="tliangso",
			personal_info=PERSONAL_INFO_IN_1
		)
...
CLASS_IN_1_TE_1_SCH_1 = ClassIn(
			teacher_id="1",
			school_id="1",
			year="2563"
		)

CLASS_IN_1_TE_1_SCH_1_PUT = ClassIn(
			teacher_id="1",
			school_id="1",
			year="2566"
		)

CLASS_IN_1_TE_1_SCH_2 = ClassIn(
			teacher_id="1",
			school_id="2",
			year="2563"
		)

CLASS_IN_1_TE_2_SCH_1 = ClassIn(
			teacher_id="2",
			school_id="1",
			year="2563"
		)
...
CLASS_1_TE_1_SCH_1_PUT = Class(
			id="1",
			teacher = TEACHER_2_SCH_1_ID_1,
			school = SCHOOL_1,
			year = "2566"
		)

CLASS_IN_2_TE_1_SCH_1 = ClassIn(
			teacher_id="2",
			school_id="1",
			year="2563"
		)
...
STUDENT_IN_3_CL_1_SCH_1_PUT = StudentIn(
			...
			school_id="1",
			intake_year="2563",
			personal_info = PERSONAL_INFO_IN_3
		)
...
class CreateHeadMasterTest(TestCase):
	...
	def test_create_headmaster_school_with_headmaster(self):
		...
		self.assertEqual(headmaster, (409, {'detail': "School already has a HeadMaster."}))
	...
	def test_create_headmaster_username_exist(self):
		...
		self.assertEqual(headmaster_2, (409, {'detail': 'This username already exist.'}))
...
class PUTHeadMasterTest(TestCase):
	...
	def test_put_headmaster(self):
		headmaster = update_headmaster(None, headmaster_id=1, payload=HEADMASTER_IN_1_SCH_1_PUT)
		self.assertEqual(headmaster,
				(
					200,
					{
	...
	def test_put_headmaster_no_headmaster(self):
		headmaster = update_headmaster(None, headmaster_id=2, payload=HEADMASTER_IN_2_SCH_1)
		self.assertEqual(headmaster, (404, {"detail":"HeadMaster does not exist."}))

	def test_put_headmaster_school_does_not_exist(self):
		headmaster = update_headmaster(None, headmaster_id=1, payload=HEADMASTER_IN_1_SCH_2)
		self.assertEqual(headmaster, (404, {"detail":"School does not exist."}))

	def test_put_headmaster_school_with_headmaster(self):
		create_school(None, payload=SCHOOL_IN_2)
		create_headmaster(None, payload=HEADMASTER_IN_2_SCH_2)
		headmaster = update_headmaster(None, headmaster_id=1, payload=HEADMASTER_IN_1_SCH_2_PUT)
		self.assertEqual(headmaster, (409, {"detail":"School already has a HeadMaster."}))
...
class PUTStudentTest(TestCase):
	...
	def test_put_student(self):
		student = update_student(None, student_id=1, payload=STUDENT_IN_3_CL_1_SCH_1_PUT)
		self.assertEqual(student,
				(
					200,
					{
