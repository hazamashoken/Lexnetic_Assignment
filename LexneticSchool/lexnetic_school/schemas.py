...
class MemberPatch(Schema):
	school_id: int = None
	personal_info: PersonalInfoPatch = None
...
class ClassOut(ModelSchema):
	...
	def resolve_students(obj):
		return [
			student for student in Student.objects.all() if student.f_class.id == obj.id
		]
	...
	def resolve_teacher_count(obj):
		return len(
			[
				...
				if teacher.member.school.id == obj.id
			]
		)
	...
	def resolve_student_count(obj):
		return len(
			[
				...
				if student.member.school.id == obj.id
			]
		)
